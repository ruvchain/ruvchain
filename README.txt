...
jar/exe/dmg package, or run "java -jar ruv-client.jar" to start the
installer. See https://bitbucket.org/JeanLucPicard/ruv/issues/283 for more
details about the IzPack installer. After installation, use the shortcuts or
desktop icons to start the Ruv server.


Using the ruv-client.zip package:

Unpack the ruv-client.zip package and open a shell in the resulting ruv
...
the ruv_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.ruv for
configuration files, logs, and the ruv_db database. Unlike run.sh, start.sh
...
message "Ruv server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:5251 ,
where the Ruv UI should now be available.
...
to the Ruv installation directory, as the use of special characters may result
...
conf/ruv-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
ruv-default.properties directly, but create a new conf/ruv.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from ruv-default.properties, the
settings in ruv.properties override those in ruv-default.properties. This way,
when upgrading the software, you can safely overwrite ruv-default.properties
with the updated file from the new package, while your customizations remain
safe in the ruv.properties file.


How to contribute?

There are many ways to contribute to Ruv. Here are some examples:
...
The Ruv software is a client-server application. It consists of a java server
...
The java process communicates with peers on port 5250 tcp by default. If you are
...
The user interface is available on port 5251. This port also accepts http API
requests which other Ruv client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
ruv_db directory. When upgrading, you should not delete the old ruv_db
...
ruv_db, except that it will take some extra time to download the blockchain
from scratch.

The default Ruv client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file ruv.log, which gets
overwritten at restart. In case of an error, the ruv.log file may contain
helpful information, so include its contents when submitting a bug report.

In addition to the default user interface at http://localhost:5251 , the
following urls are available:

http://localhost:5251/test - a list of all available http API requests, very
...
http://localhost:5251/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:5251/doc - a javadoc documentation for client developers who
...
ruv.jar file using the enclosed jar.sh script, and then ruv.jar should be

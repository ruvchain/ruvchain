...
public abstract class ShufflingTransaction extends TransactionType {
    ...
    private final static Fee SHUFFLING_PROCESSING_FEE = new Fee.ConstantFee(10 * Constants.ONE_RUV);
    private final static Fee SHUFFLING_RECIPIENTS_FEE = new Fee.ConstantFee(11 * Constants.ONE_RUV);
    ...
    public static final TransactionType SHUFFLING_CREATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws RuvException.ValidationException {
            ...
            switch (holdingType) {
                case RUV:
                    if (amount < Constants.SHUFFLING_DEPOSIT_NQT || amount > Constants.MAX_BALANCE_NQT) {
                        throw new RuvException.NotValidException("Invalid NQT amount " + amount
                ...
                case ASSET:
                    ...
                    if (asset == null) {
                        throw new RuvException.NotCurrentlyValidException("Unknown asset " + Long.toUnsignedString(attachment.getHoldingId()));
                    }
                    if (amount <= 0 || amount > asset.getInitialQuantityQNT()) {
                        throw new RuvException.NotValidException("Invalid asset quantity " + amount);
                    }
                    break;
                case CURRENCY:
                    ...
                    if (!currency.isActive()) {
                        throw new RuvException.NotCurrentlyValidException("Currency is not active: " + currency.getCode());
                    }
                    if (amount <= 0 || amount > Constants.MAX_CURRENCY_TOTAL_SUPPLY) {
                        throw new RuvException.NotValidException("Invalid currency amount " + amount);
                    }
                    break;
                default:
            ...
            if (attachment.getParticipantCount() < Constants.MIN_NUMBER_OF_SHUFFLING_PARTICIPANTS
                ...
                throw new RuvException.NotValidException(String.format("Number of participants %d is not between %d and %d",
            ...
            if (attachment.getRegistrationPeriod() < 1 || attachment.getRegistrationPeriod() > Constants.MAX_SHUFFLING_REGISTRATION_PERIOD) {
                throw new RuvException.NotValidException("Invalid registration period: " + attachment.getRegistrationPeriod());
            }
        }
    ...
    public static final TransactionType SHUFFLING_REGISTRATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws RuvException.ValidationException {
            ...
            if (shuffling == null) {
                throw new RuvException.NotCurrentlyValidException("Shuffling not found: " + Long.toUnsignedString(attachment.getShufflingId()));
            }
            ...
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
                throw new RuvException.NotCurrentlyValidException("Shuffling state hash doesn't match");
            }
            if (shuffling.getStage() != Shuffling.Stage.REGISTRATION) {
                throw new RuvException.NotCurrentlyValidException("Shuffling registration has ended for " + Long.toUnsignedString(attachment.getShufflingId()));
            }
            if (shuffling.getParticipant(transaction.getSenderId()) != null) {
                throw new RuvException.NotCurrentlyValidException(String.format("Account %s is already registered for shuffling %s",
                        Long.toUnsignedString(transaction.getSenderId()), Long.toUnsignedString(shuffling.getId())));
            }
            if (Ruv.getBlockchain().getHeight() + shuffling.getBlocksRemaining() <= attachment.getFinishValidationHeight(transaction)) {
                throw new RuvException.NotCurrentlyValidException("Shuffling registration finishes in " + shuffling.getBlocksRemaining() + " blocks");
            }
        }
    ...
    public static final TransactionType SHUFFLING_PROCESSING = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws RuvException.ValidationException {
            ...
            if (shuffling.getStage() != Shuffling.Stage.PROCESSING) {
                throw new RuvException.NotCurrentlyValidException(String.format("Shuffling %s is not in processing stage",
            ...
            if (participant == null) {
                throw new RuvException.NotCurrentlyValidException(String.format("Account %s is not registered for shuffling %s",
                        Long.toUnsignedString(transaction.getSenderId()), Long.toUnsignedString(shuffling.getId())));
            }
            if (!participant.getState().canBecome(ShufflingParticipant.State.PROCESSED)) {
                throw new RuvException.NotCurrentlyValidException(String.format("Participant %s processing already complete",
                        Long.toUnsignedString(transaction.getSenderId())));
            }
            if (participant.getAccountId() != shuffling.getAssigneeAccountId()) {
                throw new RuvException.NotCurrentlyValidException(String.format("Participant %s is not currently assigned to process shuffling %s",
                        Long.toUnsignedString(participant.getAccountId()), Long.toUnsignedString(shuffling.getId())));
            }
            if (participant.getNextAccountId() == 0) {
                throw new RuvException.NotValidException(String.format("Participant %s is last in shuffle",
                        Long.toUnsignedString(transaction.getSenderId())));
            }
            byte[] shufflingStateHash = shuffling.getStateHash();
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
            ...
            if (data == null && Ruv.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
            ...
            if (data != null) {
                if (data.length != participant.getIndex() + 1 && data.length != 0) {
                    throw new RuvException.NotValidException(String.format("Invalid number of encrypted data %d for participant number %d",
                            data.length, participant.getIndex()));
                }
                byte[] previous = null;
                for (byte[] bytes : data) {
                    if (bytes.length != 32 + 64 * (shuffling.getParticipantCount() - participant.getIndex() - 1)) {
                        throw new RuvException.NotValidException("Invalid encrypted data length " + bytes.length);
                    }
                    if (previous != null && Convert.byteArrayComparator.compare(previous, bytes) >= 0) {
                        throw new RuvException.NotValidException("Duplicate or unsorted encrypted data");
                    }
                    previous = bytes;
                }
            }
    ...
    public static final TransactionType SHUFFLING_RECIPIENTS = new ShufflingTransaction() {
        ...
        @Override
        Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer) throws RuvException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws RuvException.ValidationException {
            ...
            if (participant.getNextAccountId() != 0) {
                throw new RuvException.NotValidException(String.format("Participant %s is not last in shuffle",
                        Long.toUnsignedString(transaction.getSenderId())));
            }
            if (!participant.getState().canBecome(ShufflingParticipant.State.PROCESSED)) {
                throw new RuvException.NotCurrentlyValidException(String.format("Participant %s processing already complete",
            ...
            if (recipientPublicKeys.length != shuffling.getParticipantCount() && recipientPublicKeys.length != 0) {
                throw new RuvException.NotValidException(String.format("Invalid number of recipient public keys %d", recipientPublicKeys.length));
            }
            ...
            for (byte[] recipientPublicKey : recipientPublicKeys) {
                if (!Crypto.isCanonicalPublicKey(recipientPublicKey)) {
                    throw new RuvException.NotValidException("Invalid recipient public key " + Convert.toHexString(recipientPublicKey));
                }
                if (!recipientAccounts.add(Account.getId(recipientPublicKey))) {
                    throw new RuvException.NotValidException("Duplicate recipient accounts");
                }
            }
        }
    ...
    public static final TransactionType SHUFFLING_VERIFICATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws RuvException.ValidationException {
            ...
            if (shuffling.getStage() != Shuffling.Stage.VERIFICATION) {
                throw new RuvException.NotCurrentlyValidException("Shuffling not in verification stage: " + Long.toUnsignedString(attachment.getShufflingId()));
            }
            ShufflingParticipant participant = shuffling.getParticipant(transaction.getSenderId());
            if (participant == null) {
                throw new RuvException.NotCurrentlyValidException(String.format("Account %s is not registered for shuffling %s",
            ...
            if (!participant.getState().canBecome(ShufflingParticipant.State.VERIFIED)) {
                throw new RuvException.NotCurrentlyValidException(String.format("Shuffling participant %s in state %s cannot become verified",
            ...
            if (participant.getIndex() == shuffling.getParticipantCount() - 1) {
                throw new RuvException.NotValidException("Last participant cannot submit verification transaction");
            }
            byte[] shufflingStateHash = shuffling.getStateHash();
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
                throw new RuvException.NotCurrentlyValidException("Shuffling state hash doesn't match");
    ...
    public static final TransactionType SHUFFLING_CANCELLATION = new ShufflingTransaction() {
        ...
        void validateAttachment(Transaction transaction) throws RuvException.ValidationException {
            ...
            if (cancellingAccountId == 0 && !shuffling.getStage().canBecome(Shuffling.Stage.BLAME)) {
                throw new RuvException.NotCurrentlyValidException(String.format("Shuffling in state %s cannot be cancelled", shuffling.getStage()));
            }
            if (cancellingAccountId != 0 && cancellingAccountId != shuffling.getAssigneeAccountId()) {
                throw new RuvException.NotCurrentlyValidException(String.format("Shuffling %s is not currently being cancelled by account %s",
            ...
            if (!participant.getState().canBecome(ShufflingParticipant.State.CANCELLED)) {
                throw new RuvException.NotCurrentlyValidException(String.format("Shuffling participant %s in state %s cannot submit cancellation",
                        Long.toUnsignedString(attachment.getShufflingId()), participant.getState()));
            }
            if (participant.getIndex() == shuffling.getParticipantCount() - 1) {
                throw new RuvException.NotValidException("Last participant cannot submit cancellation transaction");
            }
            byte[] shufflingStateHash = shuffling.getStateHash();
            if (shufflingStateHash == null || !Arrays.equals(shufflingStateHash, attachment.getShufflingStateHash())) {
                throw new RuvException.NotCurrentlyValidException("Shuffling state hash doesn't match");
            }
            Transaction dataProcessingTransaction = TransactionDb.findTransactionByFullHash(participant.getDataTransactionFullHash(), Ruv.getBlockchain().getHeight());
            if (dataProcessingTransaction == null) {
                throw new RuvException.NotCurrentlyValidException("Invalid data transaction full hash");
            }
            ...
            if (!Arrays.equals(shufflingProcessing.getHash(), attachment.getHash())) {
                throw new RuvException.NotValidException("Blame data hash doesn't match processing data hash");
            }
            byte[][] keySeeds = attachment.getKeySeeds();
            if (keySeeds.length != shuffling.getParticipantCount() - participant.getIndex() - 1) {
                throw new RuvException.NotValidException("Invalid number of revealed keySeeds: " + keySeeds.length);
            }
            for (byte[] keySeed : keySeeds) {
                if (keySeed.length != 32) {
                    throw new RuvException.NotValidException("Invalid keySeed: " + Convert.toHexString(keySeed));
                }
            }
        }

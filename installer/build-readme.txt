Building the RUV installer

Note: all pathes are relative to the git repository folder (ruv-private)
...
Test X11 installer: java -jar ruv-client-<version>.jar
Test console installer: java -jar ruv-client-<version>.jar -console
...
Edit mac-release-package.sh and replace "Developer ID Application: Stichting RUV (YU63QW5EFW)" with your developer signature (you can also remove -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting RUV (YU63QW5EFW)" to generate an unsigned package)
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your ruv-private git repository preserving the folder structure
...
Test the installer using Java: java -jar ruv-client-<version>.jar
Test the windows executable: ruv-client-<version>.exe
...
ruv.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
